    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
                    ...
                    if (isset($_POST['nombre'])) { //si existe en el html el id 'nombre' recoge su valor y ponlo en la siguiente variable
                        $nombre = $_POST['nombre'];
                    }
                    ...
                    $sqldni = "SELECT `Id` FROM `profesores` WHERE `Nombre` = '$nombre' AND `Apellidos` = '$apellidos'"; //sentencia sql para saber el id de la persona segun su nombre completo
                    $resultadoIndice = mysqli_query($conn,$sqldni);


                      if (mysqli_num_rows($resultadoIndice)>0) { // si existe alguien con ese nombre y apellido
                          $etc=mysqli_fetch_assoc($resultadoIndice); //crea un array con todos los id que coincidan con ese nombre y apellido, retorna null si no hay nada
                          $id = $etc['Id'];
                      }
                      else{
                        echo "No existe ese nombre con esos apellidos.";
                          return;
                    }
                    ...
                    if (isset($_POST['valorNuevo'])) {
                        $valor = $_POST['valorNuevo'];
                    }


                    if($atributo == "Cod_Departamento"){ //si ponen mas d un 6 en codigo departamento dará error, puesto que solo hay 6 departamentos
                      if($valor > 6){
                        echo "El codigo de departamento no existe";
                        return;
                      }
                    }

                    $sql = "UPDATE profesores SET $atributo = '$valor' WHERE Id = $id"; //sentencia para actualizar el registro

                    if (mysqli_query($conn, $sql)) { // si se actualiza el registro correctamente
