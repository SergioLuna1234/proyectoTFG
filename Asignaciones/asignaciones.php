...
<head>
    ...
    <!-- ESTILOS DE LA TABLA-->
    <link href="asignaciones.css" rel="stylesheet" type="text/css">
</head>
            ...
            <div class="container-fluid">
                <div>
                    <form action="asignaciones.php" method="post">
                        <table>
                            <tr>
                                <th colspan="3">Buscar profesores en Asignación</th>
                            </tr>
                            <tr><td>Dia:
                                    <select name="dia" id="dia" onchange="buscar()"><option value="TODOS">TODOS</option>
                                        <?php
                                        include "conexionBBDD.php";
                                        $sqlDias="SELECT DISTINCT Dia from horario";
                                        $resultadoDia = mysqli_query($conn,$sqlDias);
                                ...
                                <td>Hora:
                                    <select name="hora" id="hora" onchange="buscar()"><option value="TODOS">TODAS</option>
                                        <?php
                                        include "conexionBBDD.php";
                                        include "formateoHoras.php";
                                        $sqlHoras="SELECT DISTINCT Hora from horario";
                                        $resultadoHora = mysqli_query($conn,$sqlHoras);
                                        if (mysqli_num_rows($resultadoHora)>0)
                                        {
                                            while($fila=mysqli_fetch_assoc($resultadoHora))
                                            {
                                                echo "<option value='".$fila['Hora']."'>".formateo($fila['Hora'])."</option>";
                                            }
                                        }
                                        ?>
                                    </select>
                                ...
                                <td>Departamento:
                                    <select name="departamento" id="departamento" onchange="buscar()"><option value="TODOS">TODOS</option>
                                        <?php
                                        include "conexionBBDD.php";
                                        $sqlDep="SELECT Id,Nombre_del_departamento from departamento";
                                        $resultadoDep = mysqli_query($conn,$sqlDep);
                                        if (mysqli_num_rows($resultadoDep)>0)
                                        {
                                            while($fila=mysqli_fetch_assoc($resultadoDep))
                                            {
...
<script>
    function buscar() {//AJAX PARA BUSCAR LAS ASIGNACIONES
        var dia = document.getElementById("dia").value;
        var hora = document.getElementById("hora").value;
        var dep = document.getElementById("departamento").value;
        var str = dia+" "+hora+" "+dep;
        ...
        xmlhttp.open("GET", "busquedaDisposicion.php?q=" + str, true);
        xmlhttp.send();
