      ...
      <div id="content">
        ...
        <div class="container-fluid">
          <!-- CONTENIDO DE LA PAGINA -->
            ...
            <br>
            <form action="eliminar_horario_resultado.php" method="post">
        <div style="Display:inline; color:red;">Aula:
            <select name="AulaID" id="AulaID" class="miSelect" onchange="mostrarCursoAsigProf(this.value,1)" required>
                <?php
                
                $sql = "SELECT Id, Nombre FROM AULAS";
                $resultado = $conn->query($sql);
                if ($resultado->num_rows > 0) { //SI CONTIENE ALGUN RESULTADO...
                    echo "<option>Seleccionar</option>";
                    while ($fila = $resultado->fetch_assoc()) {
                        $idAula = $fila['Id'];
                        $NombreAula = $fila['Nombre'];
                        echo "<option value='".$idAula."'>$NombreAula</option>";
                    }
                }
                $resultado->free();
            ...
            &nbsp;&nbsp;&nbsp;Dia:&nbsp;
            <select name="Dia" id="Dia" class="miSelect" onchange="mostrarCursoAsigProf(this.value,2)" required>
                <?php
                
                $sql = "SELECT DISTINCT Dia FROM HORARIO";
                $resultado = $conn->query($sql);
                if ($resultado->num_rows > 0) { //SI CONTIENE ALGUN RESULTADO...
                    echo "<option>Seleccionar</option>";
                    while ($fila = $resultado->fetch_assoc()) {
            ...
            &nbsp;&nbsp;&nbsp;Hora:&nbsp;
            <select name="Hora" id="Hora" class="miSelect" onchange="mostrarCursoAsigProf(this.value,3)" required>
                <?php
                
                include 'victor/funcionFormateoHoras.php';
                $sql = "SELECT DISTINCT Hora FROM HORARIO";
                $resultado = $conn->query($sql);
                if ($resultado->num_rows > 0) { //SI CONTIENE ALGUN RESULTADO...
                    echo "<option>Seleccionar</option>";
                    while ($fila = $resultado->fetch_assoc()) {
            ...
            <script>
                ...
                function mostrarCursoAsigProf(resultado, opcion) {
switch(opcion){
    case 1:{
        AulaID=resultado;
    ...
    case 2:{
        Dia=resultado;
        if(document.getElementById("AulaID").value==""){
            AulaID="~";
        }
        else{
...
Hora+=" ";
                    if (resultado=="") {
    document.getElementById("CursProfAsig").innerHTML="";
    return;
  }
                ...
                document.getElementById("CursProfAsig").innerHTML="";
                document.getElementById("CursProfAsig").innerHTML=this.responseText;
                }
                }
                    
                    
                    
                xmlhttp.open("GET","busquedaCursAsigProfajax.php?respuesta="+AulaID+Dia+Hora,true);
                xmlhttp.send();    
                }
            </script>
            </div>
            <div id="CursProfAsig" style="Display:inline;"></div>
