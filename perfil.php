      ...
      <div id="content">
        ...
        <div class="container-fluid">
          <!-- CONTENIDO DE LA PAGINA -->
            ...
            {
                $usuario=$_SESSION["nombre_usuario"]["name"];
                $sql = 'SELECT PROFESORES.usuario FROM PROFESORES INNER JOIN CARGOS ON PROFESORES.Id = CARGOS.Cod_profesor WHERE CARGOS.Nombre="'.$usuario.'"';
                $resultado = $conn->query($sql);
                
                if ($resultado->num_rows > 0) 
                {
                    if($usuario==="jefedeestudios")
                    {
                ...
                $sql1 = 'SELECT PROFESORES.Nombre AS Nombre, PROFESORES.Apellidos, PROFESORES.Fecha_de_nacimiento, PROFESORES.Numero_de_horas, DEPARTAMENTO.Nombre_del_departamento FROM (DEPARTAMENTO INNER JOIN PROFESORES ON DEPARTAMENTO.Id = PROFESORES.Cod_Departamento)  WHERE PROFESORES.usuario= "'.$usuario.'"';
                $resultado1 = $conn->query($sql1);
            
                if ($resultado1->num_rows > 0) 
                {
                    $campo=mysqli_fetch_fields($resultado1);
                    while ($fila=$resultado1->fetch_assoc()) {
                        $date = date('d/m/Y', strtotime($fila['Fecha_de_nacimiento']));
                        echo "<h3> Nombre: ".$fila['Nombre']." &nbsp;&nbsp;&nbsp;&nbsp; Apellidos: ".$fila['Apellidos'];
                        if($cargo==="Administrador" || $cargo==="Jefe de Estudios")
                        {
                        echo " (".$cargo.")";    
                        }
                        echo "</h3><br>";
                        echo "<h4> Fecha de nacimiento: ".$date." &nbsp;&nbsp;&nbsp;&nbsp; Numero de horas: ".$fila['Numero_de_horas']."</h4><br>";
                        echo "<h4>Departamento: ".$fila['Nombre_del_departamento'];
 
                    }
                }
            
                $sql2= 'SELECT DEPARTAMENTO.Nombre_del_departamento, PROFESORES.Nombre FROM DEPARTAMENTO INNER JOIN PROFESORES ON DEPARTAMENTO.Id = PROFESORES.Cod_Departamento WHERE PROFESORES.Id=Jefe_departamento AND profesores.usuario="'.$usuario.'"';
                $resultado2 = $conn->query($sql2);
            
                if ($resultado2->num_rows > 0) 
                {
                    $campo=mysqli_fetch_fields($resultado2);
                    while ($fila=$resultado2->fetch_assoc()) {
                        echo "(Jefe de departamento)";
                    }
                }
                    echo "</h4><br>";
                    echo "<h4>Asignaturas que impartes</h4><br>";
                $sql3='SELECT ASIGNATURA.Nombre_de_la_asignatura, CURSOS.Curso, CURSOS.Nombre FROM PROFESORES INNER JOIN ((CURSOS INNER JOIN ASIGNATURA ON CURSOS.Id = ASIGNATURA.Cod_curso) INNER JOIN Rel_Prof_Asig ON ASIGNATURA.Id = Rel_Prof_Asig.Cod_Asig) ON PROFESORES.Id = Rel_Prof_Asig.Cod_Prof WHERE profesores.usuario="'.$usuario.'"';
                $resultado3 = $conn->query($sql3);
            
                if ($resultado3->num_rows > 0) 
                {
                    echo "<center><table><tr><th>Asignaturas</th><th>Curso</th></tr>";
                        
                    $campo=mysqli_fetch_fields($resultado2);
                    while ($fila=$resultado3->fetch_assoc()) {
                        echo "<tr><td>".$fila["Nombre_de_la_asignatura"]."</td><td>".$fila["Curso"]."º".$fila["Nombre"]."</td></tr>";
                    }
                    echo"</table></center>";
                }
            ...
            <br>
            <h6>Si algun dato esta erroneo. Por favor, contactar con jefatura de estudios o el administrador</h6>
